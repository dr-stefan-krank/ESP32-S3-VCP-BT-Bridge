...
#define FTDI_READ_REQ \
    (USB_BM_REQUEST_TYPE_TYPE_VENDOR | USB_BM_REQUEST_TYPE_DIR_IN)
#define FTDI_WRITE_REQ \
    (USB_BM_REQUEST_TYPE_TYPE_VENDOR | USB_BM_REQUEST_TYPE_DIR_OUT)

namespace esp_usb {

// Angepasster Konstruktor für Multi-VID
FT23x::FT23x(uint16_t vid, uint16_t pid,
             const cdc_acm_host_device_config_t *dev_config,
             uint8_t interface_idx)
    : intf(interface_idx),
      user_data_cb(dev_config->data_cb),
      user_event_cb(dev_config->event_cb),
    ...
    err = this->send_custom_request(FTDI_WRITE_REQ, FTDI_CMD_RESET, 0,
                                    this->intf + 1, 0, NULL);
    if (err != ESP_OK) {
        throw(err);
    }
...
esp_err_t FT23x::line_coding_set(cdc_acm_line_coding_t *line_coding) {
    ...
    if (line_coding->dwDTERate != 0) {
        ...
        ESP_RETURN_ON_ERROR(
            this->send_custom_request(FTDI_WRITE_REQ, FTDI_CMD_SET_BAUDRATE,
                                      wValue, wIndex, 0, NULL),
            "FT23x", );
    }

    if (line_coding->bDataBits != 0) {
        const uint16_t wValue = (line_coding->bDataBits) |
                                (line_coding->bParityType << 8) |
                                (line_coding->bCharFormat << 11);
        return this->send_custom_request(FTDI_WRITE_REQ, FTDI_CMD_SET_LINE_CTL,
...
esp_err_t FT23x::set_control_line_state(bool dtr, bool rts) {
    ESP_RETURN_ON_ERROR(
        this->send_custom_request(FTDI_WRITE_REQ, FTDI_CMD_SET_MHS,
                                  dtr ? 0x11 : 0x10, this->intf, 0, NULL),
        "FT23x", );  // DTR
    return this->send_custom_request(FTDI_WRITE_REQ, FTDI_CMD_SET_MHS,
                                     rts ? 0x21 : 0x20, this->intf, 0,
                                     NULL);  // RTS
}
...
bool FT23x::ftdi_rx(const uint8_t *data, size_t data_len, void *user_arg) {
    ...
    if (data_len > 2) {
        return this_ftdi->user_data_cb(&data[2], data_len - 2,
                                       this_ftdi->user_arg);
    }
    return true;
}

void FT23x::ftdi_event(const cdc_acm_host_dev_event_data_t *event,
                       void *user_ctx) {
    FT23x *this_ftdi = (FT23x *)user_ctx;
    this_ftdi->user_event_cb(event, this_ftdi->user_arg);
}

int FT23x::calculate_baudrate(uint32_t baudrate, uint16_t *wValue,
                              uint16_t *wIndex) {
#define FTDI_BASE_CLK (3000000)
    ...
    if (baudrate > 2000000) {
        ...
        *wIndex = 0;
        ...
        const float ftdi_fractal[] = {0,     0.125, 0.25,  0.375, 0.5,
                                      0.625, 0.75,  0.875, 1};
        const uint8_t ftdi_fractal_bits[] = {0,    0x03, 0x02, 0x04,
                                             0x01, 0x05, 0x06, 0x07};
        ...
        for (ftdi_fractal_idx = 0;
             ftdi_fractal[ftdi_fractal_idx] <= divider_fractal;
             ftdi_fractal_idx++) {
        };
        ...
        int diff1 =
            ...
            (int)(FTDI_BASE_CLK /
                  (divider_n +
                   ftdi_fractal[ftdi_fractal_idx]));  // Greater than required
                                                      // baudrate
        int diff2 = (int)(FTDI_BASE_CLK /
                          (divider_n + ftdi_fractal[ftdi_fractal_idx - 1])) -
                    baudrate;  // Lesser than required baudrate
        ...
        baudrate_real = FTDI_BASE_CLK / (float)((float)divider_n +
                                                ftdi_fractal[ftdi_fractal_idx]);
        *wValue = ((0x3FFFF) & divider_n) |
                  (ftdi_fractal_bits[ftdi_fractal_idx] << 14);
        *wIndex = ftdi_fractal_bits[ftdi_fractal_idx] >> 2;
    }
    ESP_LOGD("FT23x", "wValue: 0x%04X wIndex: 0x%04X", *wValue, *wIndex);
    ESP_LOGI("FT23x", "Baudrate required: %" PRIu32 ", set: %d", baudrate,
             baudrate_real);
