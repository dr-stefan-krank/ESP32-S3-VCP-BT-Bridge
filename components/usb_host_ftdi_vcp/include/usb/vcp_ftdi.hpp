...
class FT23x : public CdcAcmDevice {
   public:
     ...
     */
    FT23x(uint16_t vid, uint16_t pid,
     ...
     * @param[in] dtr Indicates to DCE if DTE is present or not. This signal
     * corresponds to V.24 signal 108/2 and RS-232 signal Data Terminal Ready.
     * @param[in] rts Carrier control for half duplex modems. This signal
     * corresponds to V.24 signal 105 and RS-232 signal Request To Send.
     * @return esp_err_t
     */
    esp_err_t set_control_line_state(bool dtr, bool rts);

    // Multi-VID Support
    static constexpr std::array<uint16_t, 2> vids = {FTDI_VID};
   ...
   private:
    ...
    static void ftdi_event(const cdc_acm_host_dev_event_data_t *event,
                           void *user_ctx);
     ...
     * UART. The value of the Baud rate divisor is an integer plus a sub-integer
     * prescaler. Allowed values for the Baud rate divisor are: Divisor = n + 0,
     * 0.125, 0.25, 0.375, 0.5, 0.625, 0.75, 0.875; where n is an integer
     * between 2 and 16384 (214).
     *
     ...
     * between 0 and 2 are not allowed. Therefore the value of the divisor
     * needed for a given Baud rate is found by dividing 3000000 by the required
     * Baud rate.
     *
     ...
     * @param[out] wIndex
     */
    static int calculate_baudrate(uint32_t baudrate, uint16_t *wValue,
                                  uint16_t *wIndex);
