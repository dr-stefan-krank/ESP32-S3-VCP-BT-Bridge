...
class CH34x : public CdcAcmDevice {
   public:
    /**
     * @brief Constructor for this CH34x driver (jetzt Multi-VID)
     *
     * @note USB Host library and CDC-ACM driver must be already installed
     *
     * @param[in] vid            VID (z.B. NANJING_QINHENG_MICROE_VID)
     ...
     */
    CH34x(uint16_t vid, uint16_t pid,
    ...
    // Multi-VID Support
    static constexpr std::array<uint16_t, 1> vids = {
        NANJING_QINHENG_MICROE_VID};
    static constexpr std::array<uint16_t, 3> pids = {CH340_PID, CH340_PID_1,
                                                     CH341_PID};
   ...
   private:
    ...
    using CdcAcmDevice::line_coding_get;  // Manufacturer doesn't provide enough
    ...
    static int calculate_baud_divisor(unsigned int baud_rate,
