...
#define CH34X_READ_REQ                                                    \
    (USB_BM_REQUEST_TYPE_TYPE_VENDOR | USB_BM_REQUEST_TYPE_RECIP_DEVICE | \
     USB_BM_REQUEST_TYPE_DIR_IN)
#define CH34X_WRITE_REQ                                                   \
    (USB_BM_REQUEST_TYPE_TYPE_VENDOR | USB_BM_REQUEST_TYPE_RECIP_DEVICE | \
...
namespace esp_usb {
CH34x::CH34x(uint16_t vid, uint16_t pid,
             const cdc_acm_host_device_config_t *dev_config,
             uint8_t interface_idx)
    : intf(interface_idx) {
    const esp_err_t err =
        this->open_vendor_specific(vid, pid, this->intf, dev_config);
    if (err != ESP_OK) {
        throw(err);
    }
}
...
esp_err_t CH34x::line_coding_set(cdc_acm_line_coding_t *line_coding) {
    ...
    if (line_coding->dwDTERate != 0) {
        ...
        if (calculate_baud_divisor(line_coding->dwDTERate, &factor, &divisor) !=
            0) {
        ...
        baud_reg_val |= BIT7;
        ESP_RETURN_ON_ERROR(
            this->send_custom_request(CH34X_WRITE_REQ, CH34X_CMD_WRITE, 0x1312,
                                      baud_reg_val, 0, NULL),
            TAG, "Set baudrate failed");
    }
    ...
    if (line_coding->bDataBits != 0) {
        ...
        ESP_RETURN_ON_ERROR(
            this->send_custom_request(CH34X_WRITE_REQ, CH34X_CMD_WRITE, 0x2518,
                                      lcr, 0, NULL),
            TAG, "Set line coding failed");
    }
...
esp_err_t CH34x::set_control_line_state(bool dtr, bool rts) {
    ...
    return this->send_custom_request(CH34X_WRITE_REQ, CH34X_CMD_MODEM_OUT,
                                     wValue, this->intf, 0, NULL);
}

int CH34x::calculate_baud_divisor(unsigned int baud_rate, unsigned char *factor,
