...
namespace esp_usb {
CP210x::CP210x(uint16_t vid, uint16_t pid,
...
esp_err_t CP210x::line_coding_get(cdc_acm_line_coding_t *line_coding) {
    ...
    uint8_t temp_data[2];
    ESP_RETURN_ON_ERROR(
        this->send_custom_request(CP210X_READ_REQ, CP210X_CMD_GET_LINE_CTL, 0,
...
esp_err_t CP210x::line_coding_set(cdc_acm_line_coding_t *line_coding) {
    ...
    if (line_coding->dwDTERate != 0) {
        ESP_RETURN_ON_ERROR(this->send_custom_request(
                                CP210X_WRITE_REQ, CP210X_CMD_SET_BAUDRATE, 0,
                                this->intf, sizeof(line_coding->dwDTERate),
                                (uint8_t *)&line_coding->dwDTERate),
                            "CP210X", );
    }
    ...
    if (line_coding->bDataBits != 0) {
        ...
        return this->send_custom_request(CP210X_WRITE_REQ,
                                         CP210X_CMD_SET_LINE_CTL, wValue,
                                         this->intf, 0, NULL);
    }
    return ESP_OK;
}
...
esp_err_t CP210x::set_control_line_state(bool dtr, bool rts) {
    ...
    return this->send_custom_request(CP210X_WRITE_REQ, CP210X_CMD_SET_MHS,
                                     wValue, this->intf, 0, NULL);
}

esp_err_t CP210x::send_break(uint16_t duration_ms) {
    ESP_RETURN_ON_ERROR(
        this->send_custom_request(CP210X_WRITE_REQ, CP210X_CMD_SET_BREAK, 1,
                                  this->intf, 0, NULL),
        "CP210x", );
